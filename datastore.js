"use strict";

const STORE = [
  {
    question:
      "How many championships have the Lakers and Celtics won combined?",
    answers: ["13", "15", "25", "33"],
    correctAnswer: "33",
    icon: "https://i.imgur.com/CfA4XVq.jpg",
    alt: "Lakers Celtics pic"
  },
  {
    question: "Which league MVP has never made the NBA Finals?",
    answers: [
      "Derrick Rose",
      "Russell Westbrook",
      "James Harden",
      "Kevin Durant"
    ],
    correctAnswer: "Derrick Rose",
    icon: "https://i.imgur.com/KlMVLts.jpg",
    alt: "Derrick Rose pic"
  },
  {
    question:
      'Who famously is quoted as saying "Its just God disgused as Micahel Jordan"?',
    answers: [
      "Charles Barkely",
      "Magic Johnson",
      "Larry Bird",
      "John Stockton"
    ],
    correctAnswer: "Larry Bird",
    icon: "https://i.imgur.com/SfHteSk.jpg",
    alt: "Larry Bird pic"
  },
  {
    question:
      "Which of the following teams have been re-branded and moved to Okalahoma?",
    answers: [
      "Seattle Supersonics",
      "Charlotte Hornets",
      "Los Angeles Spark",
      "New Jersey Nets"
    ],
    correctAnswer: "Seattle Supersonics",
    icon: "https://i.imgur.com/seSLhot.jpg",
    alt: "seattle supersonics icon"
  },
  {
    question:
      "Who is he current leader in most three-point shots made in league history?",
    answers: ["Steph Curry", "Ray Allen", "Vince Carter", "Jamal Crawford"],
    correctAnswer: "Ray Allen",
    icon: "https://i.imgur.com/Ic3K7Yr.jpg",
    alt: "hat icon"
  }
];
